  ...
  <div>
    ...
    <div class="app-container calendar-list-container">
      ...
      <el-table :data="tableData" v-loading.body="listLoading" element-loading-text="给我一点时间" stripe border fit
        ...
        <el-table-column align="center" prop="ret" label="审批" width="160"></el-table-column>

        <el-table-column align="center" prop="gold_t" label="金币" width="160"></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      handleEditClick(val) {
        ...
        changeState(this.chargeForm).then(response => {
          this.tableData.forEach(td => {
            if (td.id === this.chargeForm.id) {
              td.ret = response.data
            }
            this.$message({
              message: '修改成功',
              type: 'success'
      ...
      fetchData() {
        this.listLoading = true;
        goldCashList(this.currentPage, this.page_size).then(response => {
          this.tableData = response.data.tableData;
          this.totalPage = response.data.totalPage;
          this.listLoading = false;
        });
